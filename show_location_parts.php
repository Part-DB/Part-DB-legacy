...
*/
    ...
    *********************************************************************************/
    ...
    $action = 'default';
    if (isset($_REQUEST['subloc_button']))      {$action = 'change_subloc_state';}
    $selected_part_id = 0;
    ...
    *********************************************************************************/
    ...
    try
    {
        ...
        $html->set_title(_('Teileansicht') . ': ' . $location->get_name());
    }
    catch (Exception $e)
    {
    ...
    *********************************************************************************/
    ...
    if ( ! $fatal_error)
    {
        try
        {
            $parts = $location->get_parts($with_sublocations, true);
            $table_loop = Part::build_template_table_array($parts, 'location_parts');
    ...
    *********************************************************************************/
    ...
    $html->set_variable('with_sublocations', $with_sublocations, 'boolean');

    if ( ! $fatal_error)
    {
        $html->set_variable('lid',                      $location->get_id(), 'integer');
        $html->set_variable('location_name',            $location->get_name(), 'string');
    ...
    *********************************************************************************/
    ...
    if ( ! $fatal_error)
        $html->print_template('show_location_parts');
