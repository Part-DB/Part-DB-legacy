...
**********************************************************************************************
    ...
    $Id: db_update.php 475 2012-07-02 16:09:22Z kami89@gmx.ch $
*/

  $sollDBVersion = 12; // Diese Version erwarten wir. Darf nur Incrementiert werden !
  ...
  */
  function doDBUpdate()
  {
    ...
    if (checkDBUpdateNeeded())
    {
      ...
      {
        return "WARNUNG: Ihre Datenbank Version ".$ver." ist neuer als diese Version von partDB unterst&uuml;tzte v".$sollDBVersion.", Update abgebrochen<br>";
      }

      $log = "your Database version ".$ver." is outdated an will now be updated to ".$sollDBVersion."<br>";
      $log .= "Get lock of database<br>";
      $query = "SELECT GET_LOCK('UpdatePartDB', 3);";  // get exclusive database access
      $result = mysql_query($query);

      if ($result == false)
      {
        $log .= "It seem that there is an database update already going on, aborting, try again later<br>";
        $log .= mysql_error()."<br>";
        return $log;
      }
        ...
        if (count($steps) > 0)
        {
          if (strlen($steps[0]) > 0)
          {
            foreach($steps as $query)
            {
              ...
              {
                $log .=  "Step: ".$query." ok<br>";
              }
            };
          }
          else
          {
            $log .= "Skipping empty update<br>";
          }
        }
        else  // Fehler, update von dieser Version ist nicht definiert
        {
          $log .= "Fehler, update von dieser Version ist nicht definiert. Check setDBUpdateSteps() f&uuml;r Version ".$ver."<br>";
          $error = 1;
        }
      ...
      $log .= "Unlocking Database<br>";
      $query = "SELECT RELEASE_LOCK('UpdatePartDB');";
      $result = mysql_query($query);
    }
    if ($error == 0)
    {
      $log .= "Update Finished<br>";
    }
    else
    {
      $log .= "Update Failed<br>";
    }
