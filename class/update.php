	...
	*/
	...
	function parse_script ()
	{
		...
		foreach ($lines as $row=>$command)
		{
			if ( $command['command'][0] <> '#' && $command['command'][0] <> ';')
			{
				...
				echo 'executing '.$command['command'].' => '.$command['option'].'<br>';
				...
				if ( $this -> errno > 0 )
				{
					...
					echo '<p style="color:red;">something getting wrong, stop executing script!</p>';
					break;
				}
			}
		}

		if ( $this -> errno == 0 ) echo '<p style="color:green;">executing of script: success</p>';
		$this -> cleanup();
	...
	private function f_copy ( $options = '' )
	{
		...
		*/
		...
		// Logfile
		echo '&nbsp;-&nbspcopying '.str_replace(BASE,'',$source).' to '.str_replace(BASE,'',$target).'<br>';
		// Kopiert Quelle nach Ziel. Bei Fehler wird Fehlercode 100 ("Failed to execute command") zurückgegeben.
		$err = copy ($source, $target);
		$this -> f_log('copy file '.str_replace(BASE,'',$source).' to '.str_replace(BASE,'',$target).': '.((!$err)?'failed':'success'));
	...
	private function f_chmod ( $options = '')
	{
		...
		*/
		...
		echo '&nbsp;-&nbsp;chmod '.BASE.'/'.$line[0].' to '.$line[1].'<br>';
	...
	private function f_sql ( $sqlscript = '' )
	{
		...
		*/
		...
		echo '&nbsp;-&nbsp;loading sql-script '.BASE.'/'.$this -> script_path.'/files/'.$sqlscript.'...<br>';
		echo '&nbsp;-&nbsp;execute sql-script '.$sqlscript.'...<br>';
	...
	private function cleanup ()
	{
		...
		*/
		...
		echo '<p>at least executing cleanup...</p>';
		foreach ( $this->backup_files as $file)
		{
			echo '&nbsp;-&nbsp;delete backup of file '.$file.'<br>';
