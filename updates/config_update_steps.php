        ...
        while ($current < $latest)
        {
            switch($current)
            {
                case 0: // this is used for all Part-DB versions before v0.3.0.RC1
                    ...
                    global $mysql_server;           $config['db']['host']                       = $mysql_server;
                    global $db_user;                $config['db']['user']                       = $db_user;
                    global $db_password;            $config['db']['password']                   = $db_password;
                    global $database;               $config['db']['name']                       = $database;
                    global $http_charset;           $config['html']['http_charset']             = $http_charset;
                    global $disable_update_list;    $config['startup']['disable_update_list']   = $disable_update_list;
                    global $disable_devices;        $config['devices']['disable']               = $disable_devices;
                    global $disable_footprints;     $config['footprints']['disable']            = $disable_footprints;
                    global $disable_help;           $config['menu']['disable_help']             = $disable_help;
                    global $disable_config;         $config['menu']['disable_config']           = $disable_config;
                    global $use_modal_dialog;       $config['popup']['modal']                   = $use_modal_dialog;
                    global $dialog_width;           $config['popup']['width']                   = $dialog_width;
                    global $dialog_height;          $config['popup']['height']                  = $dialog_height;
                    global $banner;                 $config['startup']['custom_banner']         = $banner;
